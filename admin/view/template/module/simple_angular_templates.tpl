            ...
            <div style="float:left;width:73%;">
                <table class="form" ng-repeat="row in setData.rows[setData.scenario]" ng-show="selected(row.type, row.id)">
                    ...
                    <tr ng-show="row.masterField">
                        ...
                        <td ng-init="row.displayWhen = !empty(row.displayWhen) ? row.displayWhen : {}">
                            <div style="overflow-y:scroll;max-height:200px;">
                                <div ng-repeat="v in getFieldValues(row.masterField)" ng-if="v.id != ''"><label><input type="checkbox" ng-change="createScenario()" value="{{v.id}}" ng-model="row.displayWhen[v.id]">{{v.text}}</label></div>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr ng-show="row.type == 'field' && row.required == 2 && row.masterField">
                        ...
                        <td ng-init="row.requireWhen = !empty(row.requireWhen) ? row.requireWhen : {}">
                            <div style="overflow-y:scroll;max-height:200px;">
