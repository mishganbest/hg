<?php
class ControllerCheckoutQuickCheckoutGuest extends Controller {
  	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if (!$json) {
				...
				$l = utf8_strlen($this->request->post['lastname']);
				if (($l < 1) || ($l > 32)) {
					$json['error']['lastname'] = $this->language->get('error_lastname');
				}

				$l = utf8_strlen($this->request->post['email']);
				if (($l > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
				...
				$l = utf8_strlen($this->request->post['address_1']);
				if (($l < 3) || ($l > 128)) {
					$json['error']['address_1'] = $this->language->get('error_address_1');
				}

				$l = utf8_strlen($this->request->post['city']);
				if (($l < 2) || ($l > 128)) {
				...
				$l = utf8_strlen($this->request->post['postcode']);
				if ($country_info && $country_info['postcode_required'] && ($l < 2) || ($l > 10)) {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/quickcheckout_guest.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/checkout/quickcheckout_guest.tpl';
			} else {
				$this->template = 'default/template/checkout/quickcheckout_guest.tpl';
			}
	...
	public function shipping() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/quickcheckout_guest_shipping.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/checkout/quickcheckout_guest_shipping.tpl';
			} else {
				$this->template = 'default/template/checkout/quickcheckout_guest_shipping.tpl';
			}
