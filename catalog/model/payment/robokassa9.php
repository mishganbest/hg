...
class ModelPaymentRobokassa9 extends Model {
	...
	private $INDEX=9;
  	...
  	public function getMethod($address, $total) {
		...
		$robokassa_status_store = $this->config->get('robokassa_status_store');
		
				
		if( ( !$STORE_ID && (
								!$this->config->get('robokassa_status') || 
								!$this->config->get('robokassa'.$this->INDEX.'_status')
							) 
			)
			||
			(  $STORE_ID && (  
								!$this->config->get('robokassa'.$this->INDEX.'_status_store_'.$STORE_ID) ||
								empty( $robokassa_status_store[$STORE_ID] )
							) 
			)
		)
		{
