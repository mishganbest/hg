...
class ModelShippingCdek extends Model {
	...
	function getQuote($address) {
		...
		if ($address['country_id'] == '' && $city != 'Алматы') { 
				$address['country_id'] = $this->config->get('config_country_id');
		} elseif ($city == 'Алматы') {
				$address['country_id'] = 109;
		}	
			...
			if (is_array($cdek_cities) && isset($cdek_cities['geonames']) && count($cdek_cities['geonames'])) {
				...
				if ($count = count($available)) {
					...
					foreach ($this->config->get('cdek_custmer_tariff_list') as $key => $tariff_info) {
						...
						$min_total = (float)$this->config->get('min_total');
						$max_total = (float)$this->config->get('max_total');
	...
	private function getResult(CalculatePriceDeliveryCdek &$calc, $total = 0) {
		...
		if ($calc->calculate() === true) {
			...
			if (!array_key_exists('cashOnDelivery', $response['result']) || ($this->config->get('cdek_cache_on_delivery') && array_key_exists('cashOnDelivery', $response['result']) && $response['result']['cashOnDelivery'] < $total)) {
