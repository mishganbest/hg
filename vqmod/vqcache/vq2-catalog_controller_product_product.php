...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		$this->data['catprod'] = $categories_info['category_id']; 
		} else {
		$this->data['catprod'] = false;
		}
		...
		if ($product_info) {
			...
			if ($product_info['podarok']) {
				$this->data['podarok'] = $product_info['podarok'];
			} else {
				$this->data['podarok'] = '';
			}
			...
			if ($product_info['quantity'] <= 0) {
				...
				$this->data['stock'] = $this->language->get('text_instock'). ' (' . $product_info['quantity'] . ' шт.)';
			} else {
    ...
    // +++ END NEW CODE
		...
		$city = $this->model_module_geo->smarty_function_get_city();
			...
			$service_centers = $this->model_catalog_product->getServiceCenters($city, $this->request->get['product_id']);
			...
			foreach ($delivery_points as $delivery_point) {
				$this->data['delivery_points'][] = array(
					...
					'time' => $delivery_point['time'],
					'phone' => $delivery_point['phone'],
					'href' => $delivery_point['photo_url']
				);
				...
				//$this->data['delivery_period_to_door'] = $delivery_point['delivery_period_to_door'];
