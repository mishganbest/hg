 ...
 * @author Sheepla (biuro{at}sheepla.pl)
 * @requirements:
...
class Sheepla {
        ...
        foreach ($array as $key => $value) {
            if (is_array($value)) {
                ...
                if (is_numeric($value) || !empty($value)) {
                    $elements[] = $this->document->createElement($key, $value);
                }
            }
        }
    ...
    public function validAccount() {
        ...
        if( isset( self::$accounts[ $this->config['key'] ] ) ) {
            
            if( 1 == self::$accounts[ $this->config['key'] ] )
                return true;
            return false;
        }
        ...
        try {
            ...
            if (isset($response['shipmentsFound'])) {
                ...
                self::$accounts[ $this->config['key'] ] = 1;
                return true;
            }
            
            self::$accounts[ $this->config['key'] ] = 0;
            return false;
    ...
    public function getShipmentTemplates() {
        ...
        throw new Exception( 'The response XML structure is wrong. No templates found' );
    }
    ...
    public function addShipmentsToOrders($order) {
        ...
        if ( sisset( $d->errors ) || !isset( $d->orders ) ) {
            throw new Exception( 'The response XML structure is wrong' );
        }
        $keys = array(
            'externalOrderId' => 'externalOrderId',
        ...
        foreach ($d->orders->order as $t) {
            $result[] = $this->mapXML($t, $keys);
            if (!$this->isArrayValid($t, $keys)) {
                throw new Exception( 'The response XML structure is wrong pleas update Client. Incomplete response.' );
            }
        }
        return $result;
    }
    ...
    public function getShipmentsList($page = 0, $limit = 25, $filter = null) {
        ...
        if (!isset($r->shipmentsFound)) {
            throw new Exception( 'There can be error on connection or withe account. No shipments found' );
        }
        ...
        foreach ($r->shipments->shipment as $t) {
            ...
            if (!$this->isArrayValid($result['shipments'][$n], $keys)) {
                ...
                throw new Exception( 'The response XML structure i invalid. Keys needed: ' . implode( ',' , $keys ) );
            }
            $n++;
        }
        return $result;
